...
int main(int argc, char *argv[])
{
   ...
   if(options.window_varied) {
       ...
       FILE *f = fopen("thisdata.txt", "wt");
       fprintf(f, "%s", "");
       fclose(f);

       for(curr_size = 1000000; curr_size <= 10000000; curr_size += 2000000) {
           if(curr_size == 3000000){
               curr_size = curr_size - 1000000;
           }
           if(curr_size > 15000000){
               break;
           }
           ...
           if(rank == 0) {
               ...
               if(options.print_rate) {
                   ...
                   int z;
                   for(z = 0; z<1000; z++){
                       FILE *f = fopen("thisdata.txt", "at");
                       fprintf(f, "%f ", bwnums[z]);
                       //fwrite(bwnums, sizeof(float), sizeof(bwnums), f);
                       fclose(f);
                   }
                   FILE *f = fopen("thisdata.txt", "at");
                   fprintf(f, "\n%s %i\n", "^^^^^^ SIZE: ", curr_size);
